...
namespace AppData.Service
{
		...
		public IEnumerable<HoadonSummaryDto> GetOrderSummary(string timeUnit)
    {
		...
		public async Task<bool> XoaHoadonAsync(int hoadonId)
		{
			try
			{
				...
				if (hoadon.Hoadonchitiets != null)
					await _hoadonRepository.DeleteHoadonchitietsAsync(hoadon.Hoadonchitiets);

				if (hoadon.Lichsuthanhtoans != null)
					await _hoadonRepository.DeleteLichsuthanhtoansAsync(hoadon.Lichsuthanhtoans);

				// Xóa hóa đơn
				await _hoadonRepository.DeleteHoadonAsync(hoadon);
		...
		{
			return _hoadonRepository.GetHoaDonDetails(idHoaDon);
		}
	}
}
